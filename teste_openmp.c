...
#define N 4000  // Tamanho da matriz
#define M 4000  // Tamanho da matriz
...
int main()
{
    // Configuração de semente para números aleatórios
    srand(time(NULL));

    // Alocação das matrizes
    float **input = alocar_matriz(N, M);
    float **output = alocar_matriz(N, M);
    float **input_copia = alocar_matriz(N, M); // Cópia para garantir dados idênticos no teste paralelo

    // Inicialização
    inicializar_matriz(input, N, M);

    // Copiar input original para input_copia para um teste justo
    for (int i = 0; i < N; i++)
        for (int j = 0; j < M; j++)
            input_copia[i][j] = input[i][j];

    printf("Iniciando processamento de matriz %d x %d (%d iteracoes)...\n\n", N, M, ITER);

    // --- EXECUÇÃO SERIAL ---
    double inicio_serial = omp_get_wtime();
    gauss_serial(input, output);
    double fim_serial = omp_get_wtime();
    double tempo_serial = fim_serial - inicio_serial;

    printf("Tempo Serial:   %.5f segundos\n", tempo_serial);

    // --- EXECUÇÃO PARALELA ---
    // Definindo número de threads (opcional, padrão é o máximo do sistema)
    // omp_set_num_threads(4);

    double inicio_paralelo = omp_get_wtime();
    gauss_paralelo(input_copia, output);
    ...
    printf("\n--- RESULTADOS PARA RELATORIO ---\n");
    printf("Threads utilizadas: %d\n", omp_get_max_threads());
    printf("Speedup:           %.2fx\n", tempo_serial / tempo_paralelo);
    printf("Eficiencia:        %.2f%%\n", (tempo_serial / tempo_paralelo) / omp_get_max_threads() * 100);
    printf("---------------------------------\n");
